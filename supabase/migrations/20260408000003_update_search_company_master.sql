-- search_company_master を配列パラメータ + AND/OR ロジック + ソート対応に更新
...
DROP FUNCTION IF EXISTS search_company_master(text, text, text, text, text, bigint, bigint, smallint, smallint, integer, integer, text, smallint, smallint, integer, integer);
  ...
  RETURN QUERY EXECUTE format($q$
    ...
    WHERE
      CASE WHEN $1 = 'OR' THEN (
        ...
        ($6 IS NOT NULL AND cm.city = $6) OR
