-- 企業DB検索に「企業DBラベル(company_db_labels: M&Aニーズあり等)」抽出を追加。
...
BEGIN
  ...
  IF (p_call_status_arr IS NOT NULL OR p_call_category_arr IS NOT NULL) AND v_org IS NOT NULL THEN
    IF p_call_category_arr IS NOT NULL AND array_length(p_call_category_arr, 1) >= 1 THEN
      v_cat_filter := format(' AND m.category_id = ANY(%L::uuid[])', p_call_category_arr);
    ELSE
      v_cat_filter := '';
    END IF;
