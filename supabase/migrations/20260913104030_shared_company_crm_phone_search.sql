-- Match stored phone punctuation and full-width/international input consistently.
CREATE INDEX company_profiles_org_phone ON public.company_profiles(org_id,(public.crm_identity_phone(phone))) WHERE phone<>'';
  ...
  EXECUTE $q$
    WITH matched AS NOT MATERIALIZED (
      ...
      FROM public.company_profiles WHERE org_id=$1 AND source_count>0 AND merged_into IS NULL
        AND ($2='' OR search_text LIKE '%'||$2||'%' OR ($10<>'' AND phone<>'' AND public.crm_identity_phone(phone)=$10)) AND ($3='' OR crm_stage=$3)
