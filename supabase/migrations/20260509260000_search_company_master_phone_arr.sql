-- =====================================================================
-- search_company_master に p_phone_patterns text[] 追加
--   電話番号の前方一致を複数パターン OR 結合で検索可能にする。
--   例: ["080","090"] → cm.phone LIKE '080%' OR cm.phone LIKE '090%'
--   既存の p_phone_pattern (単数) は後方互換のまま残す。
--   旧35引数版もあわせて DROP（オーバーロード並存防止）。
-- =====================================================================
...
BEGIN
  ...
  IF NOT v_use_semantic AND v_count <= 100000 THEN
...
DROP FUNCTION IF EXISTS public.search_company_master(
  ...
  p_keyword_arr text[], p_query_embedding vector, p_industry_or_mode boolean, p_city_arr text[]
);
