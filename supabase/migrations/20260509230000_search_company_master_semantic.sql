-- =====================================================================
-- search_company_master を A+B+C 対応に拡張
--   A: p_keyword_arr text[]  → 複数キーワード OR で
--      company_name/business_description ILIKE
--   B: 既存 p_saibunrui_arr で AI が直接細分類選択（変更なし、AI側で扱う）
--   C: p_query_embedding vector(1536) → pgvector 意味検索 + 既存フィルタの併用
--
--   既存の p_keyword (単数) は後方互換のためそのまま残す。
--   semantic 検索が指定されたとき:
--     - ORDER BY は cosine distance になる（p_sort_col / p_sort_dir は無視）
--     - HNSW index を使うため index scan 抑制は適用しない
-- =====================================================================
