-- Fix: 東京都 + 代表者年齢70歳以上 でタイムアウトする問題
-- 原因: count(*) OVER() が全結果セットの物理化を強制し、LIMIT が効かない
-- 対策:
--   1. COUNT を別クエリに分離（データクエリは LIMIT で早期停止可能に）
--   2. (prefecture, representative_age) 複合インデックス追加
--   3. net_income_k インデックス追加（ソート用）

-- ============================================================
-- 新規インデックス
-- ============================================================
CREATE INDEX IF NOT EXISTS idx_cm_pref_age
  ON company_master (prefecture, representative_age)
  WHERE representative_age IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_cm_net_income
  ON company_master (net_income_k)
  WHERE net_income_k IS NOT NULL;

-- ============================================================
-- RPC: COUNT と DATA を分離して高速化
-- ============================================================
...
BEGIN
  ...
  -- ── Step 1: COUNT のみ（window function 不使用） ──
  EXECUTE $q$
    SELECT count(*)
  ...
  -- ── Step 2: ページ分のデータのみ取得（count(*) OVER() 不使用） ──
