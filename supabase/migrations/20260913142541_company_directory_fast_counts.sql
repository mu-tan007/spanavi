...
CREATE INDEX company_directory_calls_cover ON public.company_profile_links(org_id,list_id,company_id,item_id) WHERE item_id IS NOT NULL;
CREATE INDEX company_directory_match_revenue ON public.company_directory_search(org_id,address_match,revenue_k,company_id);
CREATE INDEX company_directory_active_profile ON public.company_profiles(org_id,id) INCLUDE(home_state,crm_stage,registry_status,owner_name,next_action_at,needs_review) WHERE source_count>0 AND merged_into IS NULL;
...
DECLARE
 ...
 use_calls boolean; no_list boolean; base_where text; count_where text; candidates text:=''; n bigint; result jsonb; q text; source_where text:='';
BEGIN
 ...
 IF NOT use_calls AND call_where<>'' THEN
  w:=w||format(' AND p.id IN (SELECT l.company_id FROM public.company_profile_links l JOIN public.call_lists cl ON cl.id=l.list_id AND cl.org_id=l.org_id LEFT JOIN public.engagements e ON e.id=cl.engagement_id AND e.org_id=cl.org_id WHERE l.org_id=%L::uuid AND l.item_id IS NOT NULL%s)',o,call_where);
 END IF;
 IF use_calls THEN
 ...
 IF p_include_count THEN
  ...
  count_where:=replace(substring(w FROM length(base_where)+1),'p.id','d.company_id');
