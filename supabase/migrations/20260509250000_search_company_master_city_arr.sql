-- =====================================================================
-- search_company_master に p_city_arr text[] 追加
--   AIチャットで複数市区町村（東京23区など）を指定するときに OR 結合で
--   ヒットさせる。既存の p_city (単数文字列、前方一致) は後方互換のまま残す。
--
--   旧34引数版もあわせて DROP（オーバーロード並存防止）。
-- =====================================================================
...
DROP FUNCTION IF EXISTS public.search_company_master(
  ...
  p_keyword_arr text[], p_query_embedding vector, p_industry_or_mode boolean
);
