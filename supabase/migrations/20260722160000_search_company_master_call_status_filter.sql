-- 企業DB検索(search_company_master)に「架電ステータス」抽出フィルタを追加。
-- 仕様: 選んだステータスの「いずれかのリストで該当」（企業名＋電話一致でMVと突合）。
--   実ステータス9種（不通/キーマン不在/受付ブロック/受付再コール/キーマン再コール/
--   アポ獲得/キーマン断り/問い合わせフォーム/除外）＋「未架電」＋「未登録」。
--   複数選択はOR。'未登録'=どのリストにも一致キーが無い（MVにNOT EXISTS）。
-- 実装: mv_company_call_status との EXISTS/NOT EXISTS。RPCは大件数時 enable_indexscan=off に
--   するため、相関でも nestloop+seqscan にならずハッシュ半結合/反結合になる形にしている。
-- パラメータは末尾に p_call_status_arr を追加（既存呼び出しは default NULL で不変）。
...
CREATE OR REPLACE FUNCTION public.search_company_master(
  ...
  p_keyword_arr text[] DEFAULT NULL::text[],
  p_query_embedding vector DEFAULT NULL::vector,
  p_industry_or_mode boolean DEFAULT false,
  p_city_arr text[] DEFAULT NULL::text[],
  p_phone_patterns text[] DEFAULT NULL::text[],
  p_call_status_arr text[] DEFAULT NULL::text[])
...
BEGIN
  ...
  -- 電話番号: 単数 (前方一致) と複数配列 (OR LIKE prefix) の両対応
  ...
  -- 架電ステータス抽出（企業DB×リスト架電履歴の横断。企業名＋電話一致でMVと突合）
  -- 選んだステータスの「いずれかのリストで該当」。'未登録'=どのリストにも一致キー無し。
  IF p_call_status_arr IS NOT NULL AND array_length(p_call_status_arr, 1) >= 1 AND v_org IS NOT NULL THEN
    ...
    IF array_length(v_cs_sel, 1) >= 1 THEN
      v_cs_frag := format(
        $f$EXISTS (SELECT 1 FROM public.mv_company_call_status m WHERE m.org_id = %L::uuid AND m.status_label = ANY(%L::text[]) AND m.company_master_id = cm.id)$f$,
        v_org, v_cs_sel);
    END IF;

    IF v_cs_unreg THEN
      v_cs_frag := COALESCE(v_cs_frag || ' OR ', '') || format(
        $f$NOT EXISTS (SELECT 1 FROM public.mv_company_call_status m WHERE m.org_id = %L::uuid AND m.company_master_id = cm.id)$f$,
        v_org);
    END IF;
