...
-- 「受付再コール」「キーマン再コール」「社長再コール（legacy）」の
-- アイテムも除外する
--
-- 背景:
--   再アプローチ候補は「過去に他者が架電/アポ取得した番号で、現在の
--   アクティブリストに在籍するもの」を出すが、最新ステータスが再コール
--   系（受付再コール / キーマン再コール）になっているアイテムは、
--   既に予定が組まれた架電対象であり「再アプローチ候補」には不適切。
--   既存「キーマン再コール超過」カードや RecallListView 等で別管理
--   される対象なので、本カードからは除外する。
--
--   社長再コールは legacy ラベル（rename_ceo_to_keyman で正規化済み
--   だが、未バックフィル分の保険として含める）。
-- ============================================================
  ...
  WITH my_org AS (SELECT get_user_org_id() AS oid),
       ...
       phone_matched AS (
         ...
         WHERE length(ai.phone_n) >= 8
           ...
           AND COALESCE(ls.status, '') NOT IN ('アポ獲得','受付再コール','キーマン再コール','社長再コール')
