...
CREATE OR REPLACE FUNCTION public.crm_identity_address(p text) RETURNS text LANGUAGE sql IMMUTABLE PARALLEL SAFE SET search_path='' AS $$
  WITH v AS (SELECT public.normalize_call_company_address(regexp_replace(trim(normalize(coalesce(p,''),NFKC)),
    '^(?:〒\s*)?[0-9]{3}[-‐‑‒–—―−ー]?[0-9]{4}[\s_]*(?=(?:北海道|東京都|京都府|大阪府|.{2,3}県))','')) t)
  SELECT CASE WHEN public.is_comparable_call_address(t) THEN t ELSE '' END FROM v;
$$;

CREATE FUNCTION public.crm_link_deleted() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
BEGIN
  PERFORM public.crm_refresh_company(OLD.company_id);
  RETURN OLD;
END; $$;
CREATE TRIGGER crm_link_deleted AFTER DELETE ON public.company_profile_links FOR EACH ROW EXECUTE FUNCTION public.crm_link_deleted();

ALTER TABLE public.company_profile_reviews DROP CONSTRAINT company_profile_reviews_status_check;
ALTER TABLE public.company_profile_reviews ADD CONSTRAINT company_profile_reviews_status_check CHECK(status IN ('pending','separate','merged'));
CREATE FUNCTION public.merge_company_profile_review(p_review_id uuid,p_keep_company_id uuid,p_note text)
...
BEGIN
  ...
  SELECT count(DISTINCT corporate_number) INTO numbers FROM public.company_profiles WHERE org_id=o AND id=ANY(ids);
...
END; $$;
REVOKE ALL ON FUNCTION public.crm_link_deleted(),public.crm_identity_address(text),public.crm_sync_source(uuid,bigint,uuid,jsonb) FROM PUBLIC,anon,authenticated;
REVOKE ALL ON FUNCTION public.merge_company_profile_review(uuid,uuid,text) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.merge_company_profile_review(uuid,uuid,text) TO authenticated;
NOTIFY pgrst,'reload schema';


CREATE OR REPLACE FUNCTION public.update_company_profile(p_company_id uuid,p_version integer,p_changes jsonb)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
...
BEGIN
  ...
  IF coalesce(p_changes->>'registry_status',p.registry_status)<>'unknown' AND trim(coalesce(p_changes->>'registry_source',p.registry_source))='' THEN
    RAISE invalid_parameter_value USING MESSAGE='登記確認の出典・根拠を入力してください'; END IF;
...
CREATE OR REPLACE FUNCTION public.resolve_company_profile_review(p_review_id uuid,p_note text)
...
CREATE OR REPLACE FUNCTION public.get_company_profile(p_company_id uuid DEFAULT NULL,p_master_id bigint DEFAULT NULL,p_item_id uuid DEFAULT NULL,p_history_offset integer DEFAULT 0)
  ...
  SELECT jsonb_build_object('profile',to_jsonb(p)-'search_text',
    ...
    'reviews',coalesce((SELECT jsonb_agg(jsonb_build_object('id',r.id,'reason',r.reason,'status',r.status,'note',r.note,'companies',
      (SELECT jsonb_agg(jsonb_build_object('id',c.id,'company_name',c.company_name,'representative',c.representative,'phone',c.phone,
        'address',c.address,'corporate_number',c.corporate_number)) FROM jsonb_array_elements_text(r.company_ids) candidate(value) JOIN public.company_profiles c ON c.id=candidate.value::uuid
        WHERE c.org_id=o)))
