-- 企業DB検索: 株主タイプの絞り込みを算出済み列 company_master.shareholder_type に切り替える。
-- 変更点は p_shareholder_type_arr の条件生成のみ（他は 20260722220000 と同一）。
-- 行ごとの classify_shareholder_type() 評価をやめ、17秒→数百ms に短縮する。
...
BEGIN
  ...
  -- 株主タイプ: 算出済み列を参照する（旧: classify_shareholder_type(cm.shareholders) の行単位評価）
  IF p_shareholder_type_arr IS NOT NULL THEN
    IF array_length(p_shareholder_type_arr, 1) = 1 THEN
      v_conds := v_conds || format('cm.shareholder_type = %L', p_shareholder_type_arr[1]);
    ELSE
      v_conds := v_conds || format('cm.shareholder_type = ANY(%L::TEXT[])', p_shareholder_type_arr);
    END IF;
  END IF;
