-- ============================================================
-- smart_queue_keyman_rejections / _ids にクライアントフィルタ + クライアント情報を追加
-- ============================================================

drop function if exists smart_queue_keyman_rejections(uuid, text[], text, integer, integer, uuid[], text[]);
drop function if exists smart_queue_keyman_rejections_ids(uuid, text[], text, uuid[], text[]);

create or replace function smart_queue_keyman_rejections(
...
as $$
  ...
  filtered as (
    select * from base
    where (p_temps        is null or temp_label = any(p_temps))
...
grant execute on function smart_queue_keyman_rejections(uuid, text[], text, integer, integer, uuid[], text[], text[]) to authenticated;

create or replace function smart_queue_keyman_rejections_ids(
  p_engagement_id  uuid   default null,
  p_getter_names   text[] default null,
  p_sort           text   default 'reject_asc',
  p_engagement_ids uuid[] default null,
...
grant execute on function smart_queue_keyman_rejections_ids(uuid, text[], text, uuid[], text[], text[]) to authenticated;
