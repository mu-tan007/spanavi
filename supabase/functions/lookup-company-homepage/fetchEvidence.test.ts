...
function response(status: number, chunks: string[], contentType = 'text/html') {
  mocks.request.mockImplementation((_url, _options, callback) => {
    const req = new EventEmitter() as EventEmitter & { end: () => void }
    req.end = () => {
      const res = Object.assign(new EventEmitter(), { statusCode: status, headers: { 'content-type': contentType }, destroy: vi.fn() })
      callback(res)
...
describe('bounded DNS-pinned evidence fetch', () => {
  ...
  it('does not follow redirects', async () => { response(302, []); expect(await fetchEvidence('https://company.co.jp/')).toBe(''); expect(mocks.request).toHaveBeenCalledTimes(1) })
