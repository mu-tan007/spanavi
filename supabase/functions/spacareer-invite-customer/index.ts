...
//   2. 既存 auth user チェック
//        - 既存 → auth.resetPasswordForEmail で再招待リンクを送信
//        - 新規 → auth.admin.inviteUserByEmail で招待メール送信（Resend 経由 noreply@spanavi.jp）
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const inviteRedirect = pickRedirect(req)

    // ────────────────────────────────────────────────────────
    // 3. auth.users: 既存検出 → なければ invite メール送信
    // ────────────────────────────────────────────────────────
    ...
    if (authUserId) {
      existingUser = true
      // 既に Auth に居る → recovery メール（パスワード再設定）で実質的な招待として扱う
      const publicClient = createClient(supabaseUrl, anonKey)
      const { error: resetErr } = await publicClient.auth.resetPasswordForEmail(rawEmail, {
        redirectTo: inviteRedirect,
      })
      if (resetErr) {
        return jsonResp({ error: `existing user reset failed: ${resetErr.message}` }, 400)
      }
    } else {
      const { data: invited, error: inviteErr } = await adminClient.auth.admin.inviteUserByEmail(
        rawEmail,
        {
          data: { name: rawName, role: 'student', invite_purpose: 'spacareer' },
          redirectTo: inviteRedirect,
        },
      )
      if (inviteErr || !invited?.user) {
        return jsonResp({ error: `invite failed: ${inviteErr?.message || 'unknown'}` }, 500)
      }
    ...
    if (existingMember) {
      ...
      // 既存メンバーが社内スタッフだった場合は受講生として上書きしない
      // （メモリ feedback_spacareer_role_exclusive.md: 兼任不可）
    ...
    console.log(
      ...
      + `(customer=${customerId}, member=${memberId}, existing=${existingUser})`
    )
...
function pickRedirect(req: Request): string {
  const origin = req.headers.get('origin') || ''
  if (origin) return `${origin}/spacareer/login`
  return 'https://spanavi.jp/spacareer/login'
}
