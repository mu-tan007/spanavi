...
// ----------------------------------------------------------------
// 第1〜8回セッション完了時の事後課題のうち「変動課題」を Claude で生成する。
// （第1回も 2026-08-21 から固定配信を廃止し、全て変動課題で作る）
...
//   - count        : 生成する変動課題の数（既定30。固定課題が残っている場合は30未満になる）
//   - fixedItems   : 既に確定している固定課題の文字列配列（重複回避のためAIに共有）
// 出力: { items: [{position, question_text, question_hint, is_required, max_length}], usage }
//
// 2026-08-21 に固定課題マスターは全て無効化したので、通常は fixedItems は空・count=30 で来る。
// 生成後はトレーナーが管理画面で手動修正し、「追加公開」ボタンで受講生に配信する。
// 生成失敗時はフロント側がモックテンプレへフォールバックする。
//
// 第1回だけは課題の性質が違う（目的地と現在地を定める設計の回）ため、
// 設計思想ブロックと text 課題の上限を切り替えている（isDesignSession）。
// ============================================================
...
`
  ...
  // 第1回は「目的地（5年後の在り方と必要な収入）と現在地を定め、3ヶ月ゴールに落とす」設計回。
  // 第2回以降の「収益化を前進させる実務行動」とは課題の性質が違うため、設計思想を切り替える。
  // （2026-08-21 むー様指示で第1回の固定26問を廃止し、AI生成に一本化したことに伴う対応）
  const isDesignSession = sessionNo === 1
  // 記述(text)課題の上限。第1回は書いて設計する回なので上限を設けない。
  const maxTextItems = isDesignSession ? count : 2

  const designBlock = isDesignSession
    ? `# 設計思想（最重要・厳守）
第1回は「5年後にどう在りたいか・そのために必要な収入はいくらか」を決め、3年→1年→3ヶ月へ逆算し、
これまでの経験を棚卸しして現在地を掴み、3ヶ月のゴールに落とす"設計の回"である。
事後課題は、セッション中に扱った内容を受講生が自分の言葉で書き切って、次回以降の行動の土台を作るためのものである。
書くこと自体が前進になるので、記述(text)課題が大半になってよい。

各課題は次の2種類のいずれか。

1. 【設計・言語化課題】(item_type="text")  ← 第1回はこれが大半でよい
   - 議事録で実際に扱った論点を1つに絞り、具体的に書き出させる。ふわっとした感想文にはしない。
   - 金額・期限・数字を必ず伴わせる。金額を書かせるときは「なぜその額か」の根拠まで書かせる。
   - 例:「5年後の平日のある1日を、朝から夜まで（どこで・誰と・何をしているか）具体的に書いてください。」
   - 例:「5年後に必要な月収を内訳5項目以上に分解し、各項目についてなぜその額かの根拠も書いてください。」
   - 例:「3ヶ月後の状態ゴールを、①武器の方向 ②最初の一歩 ③週のリズムと数字 の3点で書いてください。」

2. 【行動課題】(item_type="checkbox")
   - 実際に手を動かす行動を1つ指示し、「実行したら Slack でトレーナーに報告・壁打ちする」形にする。
   - 対象は1〜2件に絞る。大量応募・大量提出は課さない。
   - 例:「こうなりたいと思える人を2人選び、その人が今に至るまでの経緯を調べて Slack でトレーナーに共有してください。」

# 絶対に作ってはいけない課題（除外）
- スクリーンショット／画面キャプチャ／一覧表／スプレッドシートURL 等、エビデンス提出のためだけの課題（面倒な事務作業になるため厳禁）。
- 数字も期限も根拠も伴わない、気持ちだけの吐き出し（例:「5年後の自分への言葉」）。書かせるなら必ず具体に落とす。
- 議事録・セッション内容を全く踏まえていない一般的な質問。
- キックオフヒアリング（70問）で既に聞いた内容、セッション感想、上記「固定課題」との重複。`
    : `# 設計思想（最重要・厳守）
...
${hasContext
  ? contextNotes
  : isDesignSession
    ? '（議事録・特記事項が提供されていません。この場合は、議事録依存の問いは作らず、5年後の在り方・必要な収入とその根拠・3ヶ月ゴールを具体的に書かせる設計課題のみを少数作ること）'
    : '（議事録・特記事項が提供されていません。この場合は、議事録依存の問いは作らず、収益化を前進させる実務行動の課題のみを少数作ること）'}

${fixedBlock}${designBlock}
...
# 各課題の属性
- item_type: ${isDesignSession
  ? '"text"（設計・言語化課題、第1回はこれが大半）/ "checkbox"（行動→Slack報告）。'
  : '"checkbox"（行動→Slack報告、大半はこれ）/ "text"（内省課題、全体で最大2問）。'}
...
- 議事録が薄い場合は無理に${count}問埋めず、質の高い課題のみ少数でよい。
- 各要素は次のキーを持つオブジェクト:
...
- position は1から連番。絵文字は使わない。${isDesignSession ? '' : 'text課題は全体で最大2問。'}
...
JSON配列だけを出力してください。`
  ...
  // 内省(text)課題は maxTextItems までに制限し、超過分は除外する（第2回以降は前進行動を最優先で2問、
  // 第1回は書いて設計する回なので上限なし）。
  let textCount = 0
