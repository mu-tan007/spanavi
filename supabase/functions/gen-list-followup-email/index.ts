...
function buildPrompt(input: any): string {
  ...
  // context block を 2 種類で出し分け
  let contextBlock = "";
  if (input.listContext) {
    const ctx = input.listContext;
    contextBlock = `- このメールが扱うリスト: 「${ctx.client_name || "—"}」向け 「${ctx.eng_name || "—"}」 案件の「${ctx.list_industry || "—"}」リスト
...
- 停滞度判定: ${ctx.stagnation ?? 0} / 5`;
    ...
    contextBlock = `- このメールが扱うクライアント: 「${ctx.client_name || "—"}」 (現ステータス: 「${ctx.status || "—"}」)
- 業種: ${ctx.industry || "—"}

【現状の数字】
- ステータスが「${ctx.status || "—"}」になってからの経過日数: ${ctx.days_since_status_change != null ? ctx.days_since_status_change + "日" : "—"}
- 過去の累計アポ数: ${ctx.past_appo_count ?? 0} 件
- 登録担当者数: ${ctx.contact_count ?? 0} 名

【ステータスの意味 (本文トーンの参考)】
- 「中期フォロー」: 一度は支援していたが、現在は中長期で関係を温める段階。ご無沙汰のご挨拶+再開打診のトーンが多い
- 「停止中」: 支援を停止中。再開検討の打診が中心
- 「準備中」: これから支援開始予定。準備状況確認や前向きなコミュニケーションが多い
- 「保留」: 一旦保留状態。状況伺いや、改めての再アプローチが多い`;
  } else {
    contextBlock = "(コンテキスト情報なし)";
  }
