...
【最重要ルール】
- 自己PRに「書かれていない」ことを、勝手に「無い」と決めつけて過度に減点しないこと。
- ただしプロフィールがある程度の分量（おおむね250字以上）書かれている場合は、該当観点への直接的な言及が無くても info_insufficient にはせず、読み取れる範囲で控えめにスコアを付けること（言及がまったく無い軸は2程度を目安に、関連しそうな経験があれば3も可）。
- info_insufficient=true にするのは、プロフィールが極端に短い・抽象的で、職種を問わず評価の手がかりがほとんど無い場合に限る。これは情報量が少ないだけの優秀な人材を短文のときに取りこぼさないための例外措置であり、長文には適用しない。
