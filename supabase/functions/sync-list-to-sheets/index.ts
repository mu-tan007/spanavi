...
function buildSheetData(list: any, items: any[], records: any[]) {
  ...
  for (const r of records) {
    ...
    weekMap[wk].calls++
    if (KEYMAN_CONNECT.has(r.status)) weekMap[wk].connected++
    if (r.status === 'アポ獲得') weekMap[wk].appo++
  }
  const weeks = Object.keys(weekMap).sort()
  const totalCalls = records.length
  const totalConnected = records.filter((r: any) => KEYMAN_CONNECT.has(r.status)).length
  ...
  reportRows.push(['週', '架電件数', 'キーマン通電数', 'キーマン通電率', 'アポ数', 'アポ率'])
  ...
  reportRows.push([`キーマン通電数: ${totalConnected}件（キーマン通電率: ${connRateTotal}）`])
...
function buildReportFormat(sheetId: number, rowCount: number, colCount: number, reportRows: any[][]) {
  ...
  // 列幅（文字が切れないよう十分な幅を確保）
  // 週, 架電件数, キーマン通電数, キーマン通電率, アポ数, アポ率
