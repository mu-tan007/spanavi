...
## 2026-08-03 の変更（むー様指示・その3）

### 月次確定（migration 20260803130000）
- `spacareer_trainer_reward_snapshots`（org_id, pay_month, member_id で一意）＋
  `spacareer_confirm_trainer_reward_month(month)` / `spacareer_recalculate_trainer_reward_month(month)`。
  どちらも SECURITY DEFINER で `spacareer_is_admin()` を内部チェック。鷲尾のJWTで拒否を確認済
- **確定解除は作らない**。直すときは再計算で確定値を作り直す（営業代行の運用に合わせる）
- 表示は `v_spacareer_trainer_monthly_display`。確定済みの月はスナップショット、未確定の月は
  都度計算を返す。本人用RPCもこのビューを見る
- 確定は月まるごと（行単位にしない）。画面は月フィルタの横に「この月を確定」／「再計算」
- **請求書作成時に自動で確定**する（運営が作った場合のみ。本人が自分の分を作っても確定しない）
- **2026-07 を確定済み**: 林 70,000／大渕 60,000／鷲尾 30,000（計 160,000・税込）。確定者は篠宮

### 自動卒業（migration 20260803140000）
- **自動卒業は既に `fn_spacareer_recalc_progress` にあった**（全コマ完了で `graduated`）。
  20260803130000 で足した「最後のコマ完了で卒業」トリガーは、同じ完了処理の後段でこの関数に
  上書きされるため機能しない。撤去した
- **応用コースは永久に卒業できない状態だった**。第1回(2)・第2回(2)の行が母数に入っており、
  実際に使うのは `oyo_start_session_no` 以降の(2)だけなので全コマ完了に到達しない。母数から除外
- 進捗率も是正（福原 41.18%→33.33%、小松 17.65%→20.00%）。既存2名は再計算済み
- 卒業時に `contract_ended_at` を入れるようにした。担当期間はこの日時で閉じる
- 応用コース(福原)で全コマ完了させるロールバック検証を実施し、卒業→進捗100%→担当期間0 を確認

## 2026-08-03 の変更（むー様指示・その2）

- 林 知佳・池田 昇太に `trainer_rewards` タブ権限を付与（シード漏れ。これで19名全員に付与済）
- **トレーナー本人が自分の報酬だけ見られるようにした**。トレーナー報酬タブを非運営が開くと
  「自分の報酬」表示に切り替わる。運営判定は `isSpacareerAdmin()`（rank=admin または小山）で
  DB の `spacareer_is_admin()` と範囲を揃えている。
  本人モードは必ず `spacareer_my_trainer_monthly()`（SECURITY DEFINER）を通す。
  ビューは security_invoker のため直接引くと、自分の担当受講生の回を代打で他トレーナーが
  実施していた場合にその人の行が欠けた数字で見えてしまう。migration 20260803120000。
  鷲尾のJWTで検証し、自分の2行だけが返ることを確認済。
