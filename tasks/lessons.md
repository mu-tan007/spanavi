# Lessons

## 2026-09-15: 保存が「AI処理でエラー」で落ちる正体はアクセストークン期限切れの401だった

- 興村さんのアポ取得報告が `POST /rest/v1/appointments` 401 で落ちたのに、画面は「AI処理でエラーが発生しました（アポ登録は完了）」と表示し、ボタンも「閉じる」に変わって再保存できなかった。行は1件も作られていない。
- supabase-js の access token 自動更新はタイマー駆動で、タブが凍結されると止まる。復帰後に期限切れトークンのまま送られ、更新が追いつくまでの数分間の読み書きが全部401になる（当日は02:09:32期限切れ→02:19:15更新の10分間）。直近24時間で call_records の登録も5件、同じ401で消えていた。
- 対策は個々の保存にリトライを足すのではなく、createClient の `global.fetch` に差す（`src/lib/authRetryFetch.js`）。401を掴んだら refreshSession して同じ要求を1回だけ再送する。全テーブル・全Function が一度に守られる。
- 失敗の表示を他の失敗と共用しない。「保存が失敗した」と「保存は済んだが後段のAIが失敗した」は利用者の次の行動が真逆になる。前者は入力を残して保存ボタンを出す。
- 症状の切り分けは `edge_logs` の `% | 401 | %` と `auth_logs` の `bad_jwt` / `token is expired` を突き合わせる。Edge Function のログだけ見ても出てこない。

## 2026-09-13: 住所フィルターを少数件で検証し、大規模リストの待ち時間を見落とした

- 条件の一致と画面遷移は79件で確認したが、数万件の全列取得と全架電履歴取得を待つため実利用で遅かった。
- 検索機能は実データの大規模リストでも、最初の検索と条件変更の待ち時間・取得量を測る。全件をブラウザーに渡す構造を先に確認する。
- 部分取得を全件として表示しない。条件、件数、架電対象、履歴の整合性を維持して高速化する。
- 索引のEXPLAINは実利用ロール・RLS込みでも確認する。計算式の索引はRLSの評価順により利用されないことがある。管理者ロールの速さだけを本番性能としない。
- まとめて取得するRPCも最大リストでAPIの時間制限を確認する。企業と各社の履歴をページ単位で取得し、並列数を制限して全ページが揃った結果だけを架電対象にする。

## 2026-09-06: カレンダーを無変更でも定期再描画していた

- ユーザーが求めたのはアポ保存・リスケ・キャンセル時の反映。60秒間隔と画面復帰時の再取得を追加し、毎回表示を消したため、閲覧中に更新が目立つ状態になった。
- 更新の契機は対象データの実際の変更に絞る。他メンバーの操作はDB変更通知で受け、クライアント・担当・表示期間・予定内容を確認してから更新する。
- 同じ対象を再取得するときは表示を維持し、取得成功後に置き換える。失敗時は更新失敗と明示して、旧表示を最新と誤認させない。

## 2026-08-25: 「DBは書き換えない」で残した旧URLが、社外に出る導線を殺していた

### 症状
アポ取得報告に貼っていた録音URLを先方が開くと必ずエラー。社内ポータルでは再生できるので、
社内の誰も気づけなかった。フラーレン様からの指摘で発覚（8/24の非公開化から丸1日以上）。

### 真因
録音をR2へ移すとき「`recording_url` は14.8万行あるので書き換えず、公開URLの形のまま
**鍵の入れ物**として使う」と決めた。この判断自体は正しい。
見落としたのは、**その列が社内の再生だけでなく、報告書に貼られて社外へ出ていた**こと。
社内には読み替え（`recordingUrl.js` / `recordingSource.ts`）があるので通る。社外には無い。

### ルール（再発防止）
- **列の値を「内部用の識別子」に格下げするときは、その列が外に出る経路を先に数える。**
  `grep` する先はコードだけでない。メール本文・報告書テンプレ・Slack投稿・PDF・CSV書き出し。
  今回は `appo_report` にそのまま埋め込まれていた
- **公開/非公開を切り替えたら、社外の目線で1回叩く。** 認証ヘッダを付けない素の `curl` で
  200が返るか。自分のログイン済みブラウザで確認しても意味がない
  （[[feedback_close_public_access_count_who_passes]] と同じ根っこ。あちらは社内、今回は社外）
- **URLの形を変えたら、鍵を取り出している場所を全部直す。** 今回は3か所あった:
  `src/lib/recordingUrl.js` / `supabase/functions/_shared/recordingSource.ts` /
  **SQLの `may_read_r2_key`**。SQLを忘れると、新しい録音が社内でも再生できなくなる
- **移設は「実体を移す」で終わりではない。** 実体はR2へ移したのに、
  `transcribe-recording` / `transcribe-and-extract` / `generate-call-report` の3つは
  Supabase Storage へ上げ続け、死んだ公開URLを返していた。**書き手を全部数えてから移す**

## 2026-06-18: `git add -A` で無関係な未コミット変更を本番commitに巻き込んだ

### 症状
オートコール修正3ファイルだけをcommitするつもりが、作業ツリーに残っていた別作業（スパキャリのコース分類/動画アップロード/マイグレーション）まで同一commitで本番mainにpushしてしまった。

### 真因
- commit前に `git status` で作業ツリーの状態を確認しなかった
- `git add -A` は対象外の既存変更も全てステージするため、共有repoでは他作業を巻き込む

### ルール（再発防止）
- commit前に必ず `git status` / `git diff --stat` で作業ツリー全体を確認する
- 自分が触ったファイルを**明示的に** `git add <path...>` する。`git add -A` / `git add .` は単独作業と確認済みの時だけ
- 巻き込みに気づいたら黙って進めず、ユーザーに本番反映可否を確認する

## 2026-05-20: Edge Function deploy 漏れで「AI出力フィールドの一部だけ空」障害

### 症状
アポ取得報告のAI自動生成で「先方のお人柄」項目だけ空になり、他の項目（面談経験/将来検討/その他）は正常に出る。

### 真因
- リモートの main では `temperature` → `personality` への field 改名 + プロンプト刷新が完了済み
- だが Supabase 本番の `transcribe-recording` Edge Function は古いdeploy版（v30, `temperature` 入出力）のまま放置
- 結果として：
  - フロントは `personality` を送り `data.personality` を読む
  - Edge は `temperature` を受けて `temperature` を返す
  - 名前ミスマッチで personality だけ空に。他の field は名前一致のため動作

### ルール（自分宛）
1. **「コードを書いた／pushした」と「本番稼働している」は別物**。Edge Function は明示 deploy が必須。CI で自動 deploy が走っていない限り、`supabase functions deploy <name>` か MCP `deploy_edge_function` を回す。
2. **field 名を rename したら必ず deploy 直後に１件流して検証**。フロントと Edge の I/F 不整合は静かに壊れる（HTTPステータスは200のまま、特定 field が空になるだけ）。
3. **「grep で見つからない」だけで「未実装」と即断しない**。`git fetch && git log --all -S "<keyword>"` でリモート/全branch履歴を確認してから判断する。今回ローカルが古かったため誤った前提で重複リネーム作業を始めかけた。
4. **rebase で複数 conflict が出たら一旦 abort して状況を読む**。「マージしてくれ」と自動で解決しようとせず、相手側が何をしているか先に把握する。今回はリモートの方が遥かに高度な実装で、僕の変更を merge していたら新仕様を破壊していた。

## 2026-06-02: 契約書 reward_table の出力で報酬体系パターンを後手で塞いだ

### 症状
formatRewardTable の修正が出るたびにユーザーから「別パターンも変だ」とフィードバックを受け続けた。
- M&A 売上高連動 → 「売上高がN円未満の会社：X万円」に対応
- 件数連動 (株式会社がんば) → 「アポ件数が1円以上4円未満の会社：1.5万円」と金額用 fmtJpYen を通してしまった
- 固定報酬 (M&A Lead 10万円) → 「-が10000.0億円未満の会社：10万円」と basis=「-」、hi=巨大値を素直に展開してしまった

### 真因
reward_types マスタには少なくとも3パターンある:
1. **金額連動** (売上高/当期純利益/営業利益 等): basis=金額の種類、tier の lo/hi/price は円
2. **件数連動** (アポ件数 等): basis=「アポ件数」、tier の lo/hi は件数、price は単価
3. **固定報酬**: basis 空 or「-」「固定」、tier 1つ、lo=null/0、hi=巨大値、price=固定額

最初に1だけ実装してしまい、新パターンが出るたびに分岐を後付け。

### ルール (自分宛)
1. **報酬体系を扱う時は必ず3パターン (固定/件数連動/金額連動) を最初から想定する**。lo/hi/price/basis の組み合わせで「これは何パターン？」を分岐してから整形する。
2. **新しい型を見たら、まず reward_types マスタを覗いて basis の値分布を確認する**。`select distinct basis from reward_types` のような事前確認を怠らない。
3. **fmtJpYen のような汎用整形関数は、basis が金額型と確定した時のみ使う**。件数/件名/固定はそれぞれ別整形。

## 2026-06-03: ハンドラを「親関数の closure」に作って子関数で呼んで ReferenceError

### 症状
ハードリロードで画面が真っ青のまま動かない。コンソールに
`Uncaught ReferenceError: handleClientNameClick is not defined`

### 真因
事業俯瞰のクライアント名リンク化で `handleClientNameClick` を
**`BusinessOverviewView` 直下** で定義したが、実際に渡す対象の `ListAnalysisTable` は
**`SectionListAnalysis` という別の関数コンポーネント** の中で呼ばれていた。
JSX のスコープ判定は親関数の closure を継承しない (Reactコンポーネントの境界で切れる) ため、
子コンポーネントからは「未定義」となって render 中に throw。
ビルド時には検出できない (実行時に到達して初めて crash する) ため、ハードリロードで一発全滅。

### ルール (自分宛)
1. **ハンドラ/値を JSX に渡す前に、その JSX を `return` している関数 (コンポーネント) と同じ closure で定義されているか必ず確認する**。
   1ファイルに複数コンポーネントが定義されている時 (Section系) は特に注意。
2. **「props で渡す」が正解。親関数で作って子関数で参照、は React では成立しない**。
3. **Hooks (useMemo/useCallback) を依存配列ごと書いて満足しない**。
   そのフックが**どの関数の中にいるか**で利用可能スコープが決まる。
4. **大きい変更を push する前に、開発サーバ (`npm run dev`) で1回ハードリロードして確認する**。
   ビルド成功=動作保証ではない。今回は build OK でも runtime で crash した。

## 2026-06-03: デモ org のデータが本番 Slack ランキングに合算された

### 症状
デモ org 構築時に `call_records.getter_name='篠宮 拓武'` で 7,150 件投入。
直後の cron 起動で `notify-ranking` が「篠宮 架電150件・キーマン接続150件・アポ取得150件」を本番 Slack に投稿。

### 真因
2つ重なった:
1. **Edge Function `notify-ranking` が org_id でフィルタしていなかった** (全 org の call_records を集計して本物 org の Slack に投稿)
2. **デモデータの getter_name に本物社員名 (`篠宮 拓武`) を入れてしまった** (ListView の manager_name デフォルト挙動と合わせるためだったが、これにより全 org 集計時に本物の篠宮さんの行と合算)

### ルール (自分宛)
1. **マルチ org 環境では、Edge Function の全クエリが特定 org に限定されているかを必ず確認する**。
   `notify-ranking` / `generate-daily-report` / `notify-team-report` / `post-*` 系を一通り見ること。slug ベース (`engagements.slug='seller_sourcing'`) のフィルタは複数 org でヒットするので注意。
2. **デモ/テスト org のテキストフィールド (getter_name / manager_name / contact_person 等) に本物社員名を絶対に入れない**。
   どこかの集計がテキスト一致でやっていれば即混入する。
3. **大規模なデータ投入を行う org を立てたら、cron 起動系の Edge Function を全部 grep して org_id 限定の有無を確認する** ことを構築前のチェックリストに入れる。

## 2026-06-03: アポ二重登録再発 (重複防止 trigger の時間窓が短すぎた)

### 症状
同一クライアント・同一企業・同一アポ日のアポが 2 行登録された。
- row1: 05:25 / sales=0 / item_id=null
- row2: 05:32 / sales=¥220,000 / item_id 付き
- 差 7 分

### 真因
`prevent_duplicate_appointment` trigger の判定窓が `created_at >= now() - interval '5 minutes'`。
ユーザーが 5 分超 (今回 7 分) かけて再操作した場合、ブロックされず重複 INSERT。
過去にも「5分窓では足りない」のサインがあったが、根本対策 (時間窓撤廃) を取らずに「5 分」のままにしていた。

### ルール (自分宛)
1. **アポ重複防止は時間窓に頼らない**。`(list_id, company_name, appointment_date)` の組み合わせで時間制限なくブロックするのが正解。
   別日のリスケや別案件は appointment_date が変わるので別レコードとして許容される。
2. **trigger だけでなく DB UNIQUE constraint** で物理的に1件しか入らないようにする方が更に堅牢
   (今回は trigger 強化のみだが、再々発したら UNIQUE 化を検討)。
3. **時間窓パラメータを修正する時は「もっと長くする」より「設計の不変条件」で考える**。
   「5分」のような魔法の数字は再発の温床。「同じ日の同じ会社のアポは1件」のような事業ルールベースで書く。

## 2026-06-08: 代理ログイン戻し忘れ自動復元が代理ログイン自体を破壊 (1e0a84a → revert)

### 症状
むー様が DealsView から「代理ログイン」ボタンを押すと、新タブが社内 Spanavi 画面 (DealsView の「案件」一覧) に着地し、他の全クライアントが選択肢に出てくる。
クライアントポータルに遷移できない＝代理ログイン機能が完全に死亡。
フラーレン側メンバーからも「ログインできない」と連絡 (auth log では login 200 が 3 回連続 = ログインしてもループしている状態)。

### 真因
コミット `1e0a84a fix(auth): 代理ログイン戻し忘れを自動検知して管理者セッションを復元` で App.jsx に追加した自動復元ロジックが、**代理ログインの新タブ着地時に意図せず発火**していた。

```js
const needRestoreClient = !loading && session && isClientRole && !inClientArea
  && readClientAdminBackup()
```

代理ログイン flow:
1. DealsView で admin session を `spanavi_admin_session_backup` に退避
2. magic link 新タブで開く → `/client` 着地予定
3. しかし「session が admin から client に切り替わる過渡期」+「ClientPortalApp が role 不一致で一瞬 `/` に Navigate する瞬間」に、App.jsx MainApp が描画されて `needRestoreClient` 全条件マッチ
4. → `setSession(admin)` で社内アカウントに「復元」されてしまう → `/dashboard` リロード

つまり「戻し忘れ」を検知するつもりが、「代理ログイン直後の正常な過渡状態」も同じパターンに見える。

### ルール (自分宛)
1. **「localStorage の状態だけで現在のユーザー意図を判定する」設計は、同じ localStorage を使う正常 flow と区別できない**。
   今回は backup 存在 = 戻し忘れと判定したが、代理ログインの新タブも同じ瞬間 backup を持っている。区別するには「backup 保存からの経過時間 (例: > 30 秒)」「現在のタブが magic link 由来か (hash に access_token があるか)」など、**時間・経路の情報を足す**必要がある。
2. **App.jsx の最上位ガードに副作用 (setSession + reload) を仕込むときは、全 auth 経路 (社内 login / クライアント代理ログイン / スパキャリ代理ログイン / magic link / password recovery) で発火条件を机上シミュレートする**。
   一つでも誤発火 flow があると本番が止まる。
3. **小山さん「/dashboard 開くとスパキャリに飛ぶ」のような UX 不都合を直すときは、root cause (新タブ magic link が同一 localStorage を上書きする) に手を入れるべき**。
   - 例: 代理ログインを Supabase クライアントの storage key を別 prefix に切る別 instance で開く
   - 例: 代理ログイン用 URL に `?impersonate=1` を付け、ClientPortalApp 側でその場で `signOut → setSession(impersonatedClient)` する
   症状側で if 文 1 個足すと別経路が壊れる。
4. **代理ログイン経路の変更は本番に出す前に必ず「自分の手でフルに動かす」**。
   今回は元タブで起きる症状 (小山さん問題) しか手で確認していなかったため、新タブ着地経路の副作用に気付かなかった。代理ログイン系の変更チェックリストに「①代理ログインを実行 ②新タブで Deals が描画される ③『社内に戻る』が機能 ④元タブで /dashboard が今までどおり動く」の 4 点を加える。

---

## 2026-06-18 ステータスボタン全停止（自動切電の同期throw）

**症状:** 架電フローでステータスボタンを押しても、ステータス保存も次企業遷移も一切起きなくなった（全停止）。

**根因:** `handleResult` 冒頭に追加した自動切電コードで `normName(...)` を呼んだが、`normName` は同ファイルの `fetchRecordingUrl` 内のローカル定義でスコープ外。`ReferenceError: normName is not defined` が**同期throw**し、以降のステータス保存・遷移処理が全て中断された。

**教訓:**
1. **ハンドラの先頭に処理を差し込む時、参照する関数/変数がそのスコープに実在するか必ず確認する**。同ファイルにある＝スコープ内、ではない（別関数内のローカル定義に注意）。grep で定義行が「どの関数の内側か」まで見る。
2. **付随処理（切電・通知・録音URL取得等）は本体処理を絶対にblockしない**。本体（ステータス保存・遷移）の前に置く副作用は try/catch で隔離する。録音URL取得が「失敗してもステータス保存に影響しない」設計になっていたのと同じ原則を、切電にも最初から適用すべきだった。
3. **本体フローに割り込むコードを足したら、ビルド成功だけでなく「その関数が最後まで走り切るか」を意識する**。ReferenceErrorはビルドを通る。

---

## 2026-06-20 招待/再設定リンクが通常ログイン画面に着地（recovery のハッシュ消失レース）

**症状:** 新メンバー（鷲尾）が招待メール／パスワード再設定リンクを踏むと、パスワード設定画面ではなく通常のログイン画面に遷移。`auth.users` は `confirmed_at`・`last_sign_in_at` が更新済（＝サーバ側ではリンク消費成功）なのに `has_password=false` のまま＝一度もパスワード設定画面に到達できていない。

**根因:** auth-js(2.95.3) は URL ハッシュ `#access_token...&type=recovery` を **Web Locks 取得後に非同期で** 読む(`GoTrueClient._initialize`)。一方この SPA は `main.jsx` で同期描画され、recovery リンク着地先 `/` は即 `App.jsx` の `<Navigate to="/login">` を描画して **ハッシュを消す**。auth-js がハッシュを読む頃にはトークンが消失 → セッション未確立・`PASSWORD_RECOVERY` 未発火 → 通常ログイン画面。invite 側は `isInviteFlow` を**モジュール読込時に同期捕捉**して耐性があったが、**recovery 側には同期捕捉が無く無防備**だった（非対称が真因）。

**対処:** `supabase.js` に `isRecoveryFlow` / `isPasswordSetupFlow` / `isAuthCallbackError` を追加し同期捕捉。`App.jsx` 最上位で、パスワード設定コールバックと分かった時点では `Routes`(=ハッシュを消す Navigate)を一切描画せず、セッション確立まで `AuthCallbackLoader` で待ってから `ResetPasswordPage` を出す。期限切れ着地(`#error=...otp_expired`)は `ExpiredLinkNotice` で案内。

**教訓（自分宛）:**
1. **SPA の `<Navigate>` は URL ハッシュを消す。auth コールバック（access_token/code をハッシュ・クエリで受ける）が着地し得るルートでは、auth-js がトークンを読み終える前に絶対に Navigate させない**。「着地を同期検知 → 確立まで待つ」をルーティングの最優先ガードに置く。
2. **auth-js の URL 処理は同期ではなく Web Locks 後の非同期**。`onAuthStateChange`/`getSession` のイベント頼みは、レンダー側がハッシュを消すと負ける。確実な着地検知は `window.location.hash` をモジュール読込時に同期で読む（`isInviteFlow` 方式）。
3. **invite で効いている同期捕捉ガードは recovery/magiclink にも横展開する**。片方だけ守ると非対称バグになる。auth 経路を足したら全 type（invite/recovery/magiclink/error）で着地挙動を机上シミュレートする（lessons #2026-06-08 と同じ原則）。

## 2026-07-22 スパキャリ新タブ(revenue)追加で「開いた瞬間に顧客一覧へ戻る」

**症状**: 新タブのボタンを押すと一瞬で既定ページ(customers)へリダイレクト。
**根因**: SpanaviApp.jsx には事業ごとに**ハードコードされたタブ許可リストが複数箇所**あり、`CAREER_TABS`(遷移ガード, 事業切替時に `!includes(currentTab)` で既定へ戻す)に新キーを入れ忘れた。
**教訓（自分宛）**: スパキャリ/ソーシングに新タブを足すときは、以下を**全て**更新する。1つでも漏れると無言でリダイレクトされる:
1. `src/constants/pageRegistry.js`（権限UIのマスタ）
2. `SpacareerAdminSidebar.jsx` の `ACTIVE_IDS` ＋ セクション定義
3. `SpanaviApp.jsx` の描画分岐（`currentTab === 'xxx'`）
4. `SpanaviApp.jsx` の **`CAREER_TABS` 遷移ガード配列**（← 今回漏れた。事業切替useEffect内）
5. `SpanaviApp.jsx` の canViewPage 既定タブ解決配列（line ~762）
6. `SpanaviApp.jsx` の キーボード循環 `tabs` 配列（line ~823）
7. `EngagementComingSoon` の除外配列（line ~1397 の `!includes` リスト）
8. DB: `member_page_permissions` シード（非admin公開時。admin限定ならバイパスで不要）
grep 一発点検: `grep -n "'templates','analytics'" src/components/SpanaviApp.jsx` で全リストを洗い出してから足す。

## 2026-07-23 追記: 新タブのハードリロードで既定ページに戻る
上記(2026-07-22)のタブ許可リストには **もう1箇所** あった: `SpanaviApp.jsx` の `_VALID_TABS`
（currentTab を localStorage から復元する際の検証リスト, line ~405）。ここに新keyが無いと
**ハードリロード時**に保存済みタブが無効判定され既定(customers)へ戻る。
→ 新タブ追加チェックリストに「_VALID_TABS」を追加。CAREER_TABS=クリック遷移、_VALID_TABS=リロードの2系統。

## 2026-08-03 累計売上のズレを「問題」として報告してしまった

**症状**: 報酬スナップショットの自動再計算を設計中、`v_member_cumulative_sales_audit` に
11名分の差分があるのを見つけて「減算漏れの可能性」として報告した。
むー様から「前言った通り、初期値として手で入力したものが反映されているから」と再度の指摘。

**根因**: `members.cumulative_sales` はシステム導入前の実績を手入力した初期値を含む。
面談済アポの合計と一致しないのが**正常**。同じ指摘を受けるのは2回目。

**教訓（自分宛）**:
1. 監査ビューやデータの不一致を見つけたら、**報告する前に「これは仕様として説明済みではないか」を疑う**。
   特に「マスターの数字 vs 明細の合計」がズレるのは、移行時の初期値という説明がつくことが多い。
2. `cumulative_sales` をアポ明細から再計算して上書きする実装は**絶対にしない**。初期値が消える。
3. 累計売上はステータス変更のたびに増減するカウンターで、`src/utils/cumulativeSales.js` の
   `cumulativeSalesDelta()` が全経路（アポ一覧の一括変更・編集モーダル2箇所・事前確認画面）に
   入っている。ここに別処理から二重に足し引きしないこと。

## 2026-08-03 遅いSQLの原因を関数コストと決めつけて外した

**症状**: 企業DB検索の statement timeout（17.9秒）を調査し、WHERE句にある
`classify_shareholder_type()`（1行0.4ms・内部でSPI問い合わせ）を犯人と断定。
算出列 `shareholder_type` に置き換えて49万行のバックフィルまで実施したが、
**16.7秒のまま何も改善しなかった**。

**根因**: 支配的だったのはCPUではなくランダムなディスク読み込み。
部分索引が `(net_income_k, revenue_k)` の2列しか持たず、43,168件を1行ずつ
本体テーブルから読んでいた（0.33ms × 43,168回 ≒ 16秒）。
`EXPLAIN (ANALYZE, BUFFERS)` の `read=26,634` を最初に見ていれば即分かった。

**教訓（自分宛）**:
1. 遅いSQLは **まず `EXPLAIN (ANALYZE, BUFFERS)`**。`read` の数 × 0.3ms が実行時間の
   どれだけを占めるか計算する。見た目が重い関数に飛びつかない。
2. `Rows Removed by Filter` が数万 かつ buffers が行数と同オーダー＝「索引→本体のランダム読み」。
   索引に絞り込み列を足して index only scan（Heap Fetches: 0）にするのが正解。
3. 対処したら **同じ計測をやり直す**。変わらなかったら見立てが違うので、そこで引き返す。
4. 被覆索引を作ったら **旧索引は必ず drop**。残すとプランナが「安く見える」旧索引を選び続ける。
5. 大量UPDATEの直後は可視性マップが未整備。`VACUUM (ANALYZE)` してから計測する（1.4秒→94ms）。

---

## 2026-08-07 アナリティクスのメンバー別架電数が全員消えた（RPCのチーム名べた書き）

**症状**: アナリティクス「チーム比較」表・事業俯瞰のメンバー表が8月に入って空になった。先月までは一覧が出ていた。

**根因**: `business_overview_member_performance` の対象メンバーが
`m.is_active = true and m.team in ('成尾', '高橋')` と**チーム名を直書き**していた。
8月のチーム改編で成尾・高橋チームの在籍者が 0 になり、返る行がゼロに。
さらに `split_part(m.name,' ',1) <> m.team` でチーム名と姓が一致する行を落としており、
**リーダー本人（浅井・瀬尾）の架電数は改編前からずっと非表示**だった。
実際には主力の浅井・瀬尾チームは5月から稼働しており、「7月まで見えていた一覧」は
成尾・高橋チームの数名だけ＝**壊れていたのに気づけない見え方**になっていた。

**対処**: 対象を「在籍かつチーム設定あり」または「期間内に架電/シフト/稼働の実績あり」に変更。
リーダー除外条件は撤廃。退職者も期間内に実績があれば出るので過去月を遡れる。

**教訓（自分宛）**:
1. **組織の実データ（チーム名・人名・拠点名）を SQL の条件に直書きしない**。
   人事異動で無言でゼロ件になる。`org_id` 直書きと同じ罠（[[feedback_rls_hardcoded_org_id]]）。
   絞りたいなら「team が設定されている」「期間内に実績がある」など**性質**で書く。
2. **「一覧が出ている」は正常の証拠にならない**。一部の行だけ出ていても気づけない。
   一覧系を直したら**全社合計と突き合わせる**（今回: 7月の表合計 27,932 = call_records の7月全件と一致）。
3. **除外条件は「誰が消えるか」を具体名で確認してから書く**。姓とチーム名の一致でリーダーを消す実装は、
   条件だけ読むと妥当に見えるが、実データに当てると狙った人以外も落ちる。

---

## 「◯◯ページと同じ形で」は、見た目だけでなく**対象範囲**も合わせる（2026-08-24）

**状況**: 「シフトページを、メンバーページと同じようにチーム別で表示して」という依頼。
チーム分けの見た目と並び順はメンバーページ（`EngagementMembersView` / `teamGroups`）に揃えたが、
**行の母集団は従来どおり `members` prop（全社の在籍者）のまま**にし、
メンバーページに載っていない他事業専任の5名を「未所属」に落として残す実装にした。

**指摘**: 「メンバーページに載っている人だけをシフトページに表示させればいい。
大渕とか林とかって、営業代行のメンバーのページには載ってないでしょ？」

**根因**: 「表から誰も消えないほうが安全」という自分の判断を優先し、
**2画面の対象範囲がズレる状態を"安全側"と誤認した**。
実際には、メンバーページに存在しない人がシフト表にだけ出るほうが運用者には不整合で、
シフトを入れる相手として誤って出てしまう。

**教訓（自分宛）**:
1. **「AページをBページと同じ形に」と言われたら、揃えるのはレイアウトだけでなく
   「誰が出るか」の定義そのもの**。並び順・見出しだけ真似て母集団を変えない。
2. **「消えると危ないから残す」を無断で足さない**。残すか外すかは判断待ちとして出す。
   今回は事前に「未所属に落として残します」と説明はしたが、選択肢として提示していなかった。
3. 対象を絞る変更は、**外れる人の具体名と、その人に紐づくデータ件数を先に出す**
   （今回: 外れる5名のシフト登録は0件と確認できたので安全に外せた）。

## 動いているシステムでは「もう全部済んだ」が成り立たない（2026-08-24）

録音のR2移設で、再生を速くするために署名を出す前の「R2に実体があるか」の
確認を外した。理由は「全件（150,800件）の突合が済んでいるので確認は無駄」。

**移設のあとも架電は続いていた。** その日の朝に録音された分はR2に無く、
「ある」前提の鍵を返してしまい、Supabaseに回る道が使われず再生できなくなった。
8時台が全滅、9時台（入口を変えた後）は再生できる、という分かりにくい壊れ方をした。

- 「移送が済んだ」は**その瞬間の話**でしかない。データが増え続ける限り、
  取りこぼしが無い状態は続かない
- 最適化のために**存在確認や後始末の道を消さない**。消してよいのは、
  そこにデータが増えないと確証があるときだけ
- 遅さの原因は測ってから外す。今回の主因はプリフライトの往復であって、
  外した確認（60〜80ms）ではなかった。**測る前に外していた**

移設のように「古いものを新しい場所へ移す」作業では、必ず
**移している間にも新しいものが増える**。入口を先に切り替えてから移すか、
移し終えたあとに増えた分を拾う手順を最初から組み込む。

## 署名付きURLは「メソッドごと」に別物（2026-08-24）

R2へ移設したあと、スパキャリの第5回の議事録が「動画ファイルの取得に失敗しました」で
止まった。原因は R2 に無かったからではなく、**存在確認のしかた**だった。

```
const url = await r2SignedUrl(key)          // GET用に署名
await fetch(url, { method: 'HEAD' })        // ← 必ず403。署名にメソッドが入っている
```

403 を「無い」と読んで Supabase 側へ回り、そちらは移設で消してあるので
「Object not found」。**R2にはちゃんと在るのに、全件が失敗していた。**
架電録音の `resolveRecordingSource` も同じ書き方で、同じく全件Supabaseへ回っていた。

- AWS署名v4（S3/R2）は**メソッドを署名対象に含む**。GET用のURLでHEADを投げない。
  存在確認をHEADでやるならHEAD用に署名し直す
- **403と404を同じ扱いにしない。** 403は「鍵が違う」、404が「無い」。
  今回は403を「無い」に丸めていたので、鍵の作り方の誤りが最後まで表に出なかった
- 移設のあと「読めない」が出たら、まず**移設先に在るか**を確かめる。
  在るなら壊れているのは読み方であって、データではない

## 同期語だけでMP3を切ってはいけない（2026-08-24）

上を直したら次は Whisper が `chunk 4: Invalid file format` で落ちた。
長い音声を4MBずつ並列で投げるために `0xFF 0xEx`（同期語）でフレーム境界を探していたが、
**この2バイトは音声データの中に4MBあたり約2000回、偶然現れる**。
偽の位置で切るとチャンクの先頭がゴミになり、ffmpeg が形式を判定できず400になる。

- ビットレートとサンプリング周波数まで読んでフレーム長を出し、
  **次のヘッダがちょうどその位置に来るかを3フレーム辿って**本物だけ採る
- 「たいてい動く」のが厄介。フレーム長は約104バイトなので偽物を掴む確率は
  1切り口あたり5%程度。6分割なら4回に1回コケる。**再実行で直る＝直っていない**
---

## 公開されていたものを閉じるときは、「誰が通れるか」を実データで数える（2026-08-24）

録音をR2へ移し、公開バケットから**署名付きURL＋権限確認**に変えた。
その権限確認 `may_read_r2_key(uid, kind, key)` を

```sql
with me as (select org_id from users where id = p_uid)
```

と書いた。**`users` に載っていない人は所属が引けず、必ず不許可になる。**

実際には稼働中のメンバー47人のうち **44人が `users` に居らず `members.user_id` にしかいなかった**。
つまり**ほぼ全員が録音を1件も再生できない状態**を1日放置した。
スパキャリの受講生17人も、同じ関門で**講義動画を1本も見られなかった**。
浅井様・日高様の申告で分かった。自分では気づけていない。

### なぜ気づけなかったか

- 移設そのもの（バイト数の突合）は丁寧に確認したが、**移設後に誰が読めるかは確認しなかった**
- 自分（管理者）のアカウントでは再生できたので「直った」と判断した。
  **自分は3人の例外のほうに入っていた**

### 次からやること

- **公開 → 非公開**に変える作業では、切り替える前に
  `select count(*) filter (where 新しい許可関数(...)) from 全ユーザー`
  を実行し、**通る人数と通らない人の名前を出す**。1人でも減るなら切り替えない
- 所属や権限を引くとき、**その情報がどの表に入っているかを1つに決めつけない**。
  spanavi では所属は `users` と `members` の両方にあり、**実務上の正は `members`**
- 動作確認を**自分のアカウントだけで済ませない**。いちばん権限の弱い人で試す

### 続き：同じ関門に、あと2つ穴が空いていた（同日）

浅井様・日高様の件を直したあと、むー様から
「営業代行のクライアントポータルでクライアントは録音を確認できる？」と聞かれて調べたら、
**同じ関数にもう2つ穴**があった。

- **クライアント28社が全員不許可。** ポータルの人は `users` にも `members` にも居ない。
  `clients.auth_user_id` で入る。**3つ目の入口を数え忘れていた**
- **アポの録音407本は社内の人でも不許可。** 鍵の出どころを `call_records` しか見ておらず、
  `appointments.recording_url` を忘れていた

つまり最初の修正は**症状が出た1件だけを直していた**。同じ形の穴を探しに行かなかった。

- 権限関数を書くときは、**入口（誰が来るか）** と **出どころ（どの表に鍵があるか）** を
  それぞれ**表を数えて列挙**してから書く。思い出しで書かない。
  入口は `users` / `members` / `clients`、鍵は `call_records` / `appointments` だった
  （`information_schema.columns` で `%recording%` を引けば10秒で分かった）
- **クライアントには組織で括って渡さない。** 括ると他社の録音まで聞けてしまう。
  既にある RLS ポリシー（`call_records_select_client`）と**同じ絞りを写す**。
  画面に出る範囲と鍵が通る範囲は、必ず一致させる
- 1つ直したら、**同じ形の穴を全部探してから報告する**

## 2026-09-04 録音が「聞けるときと聞けない時がある」

**症状**: クライアントから「携帯で聞けるときと聞けない時がある」。調べると原因が3つ重なっていた。

1. **移設で旧URLが死んだ** — 8/23〜24にR2へ移してバケットを消したため、
   それ以前にSlackへ貼った `/storage/v1/object/public/recordings/` が全部400。
   DBは貼り替えたが、**外に出た文面は直せない**（436件・37社）
2. **中身はMP3なのに `audio/mp4` を名乗っていた** — 実体の先頭は `FF F3`。
   鍵の名前も `.mp4`。パソコンは中身を見て鳴らすので気づけず、iOSだけ落ちる
3. **期限つきの転送先がCloudflareにキャッシュされていた** — 共有URLが `.mp4` で
   終わるため静的ファイル扱い。1時間で切れる署名を抱えたまま302が居座り、
   時間が経つと403（`CF-Cache-Status: HIT` / `Age: 546` で発覚）

**自分への規則**:
- 「サーバーが正しく返す」と「相手の端末で鳴る」は**別**。寛容な環境だけで確かめない
- 音や動画が鳴らないときは、**まず実体の先頭バイトを見る**。名前と型は当てにならない
- **期限つきのURLへ転送するなら、その転送は必ず `no-store`**。
  期限より長くキャッシュされたら、そこが壊れる
- `403` を「無い」と読まない。R2のライフサイクルも `lifecycle-get` の403を
  「規則なし」と読んで誤った（実体の `x-amz-expiration` を見れば1手だった）
- 外に出す文字列は二度と直せない。**出す前に、その形で永久に有効かを決めきる**

---

## 2026-09-07 受講生を切り替えてもタブが残り、実施していない回が完了になった（スパキャリ）

**症状**: 古藤さんの進捗が「キックオフ・第1回・第7回が完了、第2回が次回」という
飛び方をしていた。実際に 9/3 に実施したのは第2回。第7回として記録され、
議事録・動画・事後課題・感想まで第7回にぶら下がっていた。

**原因**: `CustomerDetail` は受講生を切り替えてもアンマウントされないのに、
開いているタブ (`tab`) を初期化していなかった。トレーナーは直前に別の受講生
（米澤さん・第7回まで到達済み）の「第7回セッション管理」を開いていたため、
古藤さんに切り替えても中身が第7回のまま表示された。さらに中央の描画は
`tab` の文字列から直接分岐しており、**タブ一覧に無い回でも描画できてしまう**。
そこで次回日時を入れて「セッション完了」を押した結果、第8回に日程が入り、
第7回が完了になった。副作用で `direct_db_access_granted_at`（第4回完了で付与）
まで先に付いていた。

**自分への規則**:
- 一覧＋詳細で**詳細側が使い回される**画面は、対象が変わったら**開いている場所を必ず初期化**する。
  データだけ差し替えて「どこを見ているか」を残すと、別人の操作画面が出る
- 表示条件で出し分けるタブ・ページは、**中身の分岐も同じ条件を通す**。
  ID文字列から直接分岐すると、出していないはずの画面が開く
- 進捗が飛んでいる報告は、まず**その行の更新時刻を並べる**。
  古藤さんは第8回(09:25:36)→第7回(09:25:39)の順で、1つの操作だと一目で分かった

## 2026-09-10 同名企業のHP誤取得

東京都の白石工務店に愛媛県の同名会社のHPを報告した。元リストは正しい住所・代表者でHP空欄だった。旧検索を実行しても誤URLをconfidence=highで返し、根拠に実際と異なる住所一致を記載した。

- **AIの確信度は本人確認の証拠にならない。** 会社名と補助情報を渡すだけでなく、公開ページ本文で事実を照合する。
- **公式サイト自身の情報であることも確認する。** 第三者の企業一覧や取引先紹介に同じ情報があっても採用しない。
- **識別情報が欠ける・一致しない場合はURLなし。** それらしい候補で空欄を埋めない。
- **非同期の検索結果は対象変更・手動編集後に反映しない。** 入力した後に消した空欄も本人の意図として保持する。
- **訂正メールだけでは保存済み報告は直らない。** 対象行の控えを取り、残っている誤URLも限定して修正する。

## 2026-09-13 住所一致検索の0件表示

- 大規模リストで0件が速く返ることだけで、利用者が必要とする住所比較ができると判断しない。リストごとの元データに会社住所・代表者自宅住所が含まれるか、比較可能件数と正の該当例を確認する。
- 判定に必要なデータ不足と、比較した結果の該当なしを画面で区別する。空の一覧だけでは理由が伝わらない。

## 2026-09-13 住所不足の調査を単一リストで終えてしまった

- あるリストに住所がないだけで新たなデータ提供が必要と結論付けない。同一企業の企業DB・別リスト・取込原本を照合し、既に持っている情報で補完できるか確認する。
- 情報を共有するときは企業名だけで紐付けない。法人番号や会社名＋補助識別子を使い、原本・出典と矛盾を保持する。

## 2026-09-13 大量名寄せのCOPYでWALが容量を圧迫

- 大量取込の前に、テーブル・索引だけでなくWAL・システム領域と管理画面の実ディスク容量を確認する。計算上の行サイズだけで安全と判断しない。
- 長大な一括トランザクションを避け、非公開の初期化期間に小分けで保存し、容量・読取専用化を各バッチ前に監視する。初期化完了まで本番の参照・同期へ切り替えない。
- 失敗したトランザクションも物理領域を残す。回収する場合は対象が今回作ったテーブルかつ0行であることを確認し、原本には触れない。

## 2026-09-13 企業DBの件数表示と一覧の待ち時間

- 企業DBの主表示は利用者が知りたい企業の総数にする。保存元や名寄せの内訳を常設カードに増やさない。
- 件数が速く返っても一覧が遅ければ検索改善は未完了。最初の行の表示までを測り、全件の架電履歴取得を画面表示の前提にしない。
# 2026-09-13 企業DBの入口と検索対象を分けない

- 共有企業を作っても、既存検索を別タブに残すと「登録企業には含まれるのに詳細条件で見つからない」が起きる。画面・検索対象・出力を同じ企業IDに揃える。
- 「どんなソースでも正しくマッピング」は完全自動の保証ではない。既知の列は候補を提示し、未知・曖昧な列は確認または元の値として保持する。提供元別の設定を再利用できるようにする。

## 2026-09-14 tasks/todo.md は追記型のログ
- `tasks/todo.md` は過去案件の記録が下に積んである。新しい案件は**先頭に追記**し、丸ごと上書きしない（今回221行を消して復元した）
