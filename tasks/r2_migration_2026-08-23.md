...
- [x] 実体が無いときは「保存期間（180日）を過ぎたため残っていません。議事録は
      ...
      `resolveSessionVideoUrl` が gone を返す

## 残っていること

- 架電録音 80GB / 150,800件 の移送（`spanavi-recordings` バケットの作成から）
  ⚠️ 講義録画と違い、本番のCTIが常時読み書きしている
- 架電録音の保存期間（6か月で良いか。商談の証跡として長く要る可能性）

## 誰が録画を取れるか（2026-08-23 時点）

| | 届くか |
|---|---|
| 受講生（17名） | **届かない。** `users` に行が無いので署名が出ない |
| 運営・トレーナー・社内メンバー（36名） | 再生できる。URLを取り出せばダウンロードもできる |
