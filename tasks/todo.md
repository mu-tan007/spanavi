...
- **クライアントポータル**（獲得アポ詳細タブ → `PlayRecordingButton` → `resolveRecordingUrl`）:
  録音つきアポを持つクライアント23社ぶんを数えて **385件中379件が再生可能**。
  残り6件はZoomの生URLのまま復旧できないもの。組織不一致で弾かれるものはゼロ。
  DBに実際に入っているURLを認証なしで3本叩いて 200 / audio/mp4 を確認（フラーレン様ぶん）
- `may_read_r2_key` は再生のたびに呼ばれるので、鍵での絞り込みを入れて 390ms → 198ms
...
## 2026-09-10 速度改善の残候補を限定して完了する
- [x] 第一段階後の候補と失敗時挙動を確認し、必須と見送りを整理。
- [x] ドシエのHP取得と独立したニュース生成の開始を重ねる。モデル・プロンプト・HP照合・認可・保存内容を維持。
- [x] 外部通信fixtureで変更前後の入力・出力、成功/失敗/HPなしを比較。全327テスト・ビルド・ドシエ関数型チェック成功。
- [ ] 本番の依存ソースを比較し、main CIで公開。配布ソースと安全な入力拒否を確認。
- [ ] 実時間の数値を推測せず残候補の見送り理由を記録して調査を閉じる。
