...
import { calcRankAndRate, getNextRankInfo } from './calculations';
...
describe('getNextRankInfo（次のランクまでの不足額）', () => {
  it('累計0円 → 次はプレイヤー、あと200万', () => {
    expect(getNextRankInfo(0)).toEqual({ nextRank: 'プレイヤー', gap: 2000000 });
  });
  it('閾値直前 → 不足額1円', () => {
    expect(getNextRankInfo(9999999)).toEqual({ nextRank: 'スーパースパルタン', gap: 1 });
  });
  it('最上位到達後は null', () => {
    expect(getNextRankInfo(10000000)).toBeNull();
    expect(getNextRankInfo(99999999)).toBeNull();
  });
  it('org_settings のランク定義を反映', () => {
    const s = { rank_definitions: JSON.stringify([
      { name: 'ゴールド', threshold: 1000000 },
      { name: 'シルバー', threshold: 0 },
    ]) };
