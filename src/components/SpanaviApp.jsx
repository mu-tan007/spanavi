...
import SpacareerHomeworkView from './spacareer/admin/homework/SpacareerHomeworkView';
...
const DEFAULT_INDUSTRY_RULES = [
  { industry: "建設", rule: "キーマンは日中現場に出ていることが多い。朝8:00〜9:30と夕方16:00〜18:00が狙い目。平日のみ稼働。", goodDays: [1,2,3,4,5], badDays: [0,6], goodHours: "8:00〜10:00, 16:00〜18:00", badHours: "10:00〜16:00", level: "specific" },
  { industry: "製造", rule: "キーマンは日中工場に出ていることが多い。朝8:00〜9:30と夕方16:00〜18:00が狙い目。平日のみ稼働。", goodDays: [1,2,3,4,5], badDays: [0,6], goodHours: "8:00〜10:00, 16:00〜18:00", badHours: "10:00〜16:00", level: "specific" },
  { industry: "物流", rule: "キーマンは日中現場・配送管理で不在が多い。朝8:00〜9:30と夕方16:00〜18:00が狙い目。平日のみ稼働。", goodDays: [1,2,3,4,5], badDays: [0,6], goodHours: "8:00〜10:00, 16:00〜18:00", badHours: "10:00〜16:00", level: "specific" },
  { industry: "IT", rule: "時間帯の差は少ないが、キーマンの在宅ワーク率が高く通電率自体が低め。平日稼働。", goodDays: [1,2,3,4,5], badDays: [0,6], goodHours: "10:00〜12:00, 14:00〜17:00", badHours: "", level: "warning" },
  ...
  useEffect(() => {
    ...
    const fallback = (() => {
      ...
      if (engSlug === 'spartia_career') {
        return ['customers','sessions','homework','social_style','ai_courses','templates','analytics','settings'].find(k => canViewPage('spartia_career', k));
      }
      return null;
    })();
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (engSlug === 'seller_sourcing') {
        ...
        cycle(['customers','sessions','homework','social_style','ai_courses','templates','analytics','settings','mypage'], currentTab, e.key, setCurrentTab);
  ...
  return (
    <div style={{ minHeight: "100vh", background: '#F3F2F2', color: color.textDark, fontFamily: font.family.sans }}>
      ...
      <main style={{ marginLeft: isMobile ? 0 : 220, paddingTop: isMobile ? 48 : 54, paddingLeft: isMobile ? 12 : 28, paddingRight: isMobile ? 12 : 28, paddingBottom: isMobile ? 72 : 24, minHeight: '100vh', width: isMobile ? '100%' : 'calc(100% - 220px)', boxSizing: 'border-box', overflowX: 'hidden' }}>
        ...
        {/* スパキャリ運営ダッシュボード 8画面（Phase 3 並列実装で全画面の中身を構築完了） */}
        ...
        {currentTab === "lists" && <ListView filteredLists={filteredLists} allLists={enrichedLists} filterStatus={filterStatus} setFilterStatus={setFilterStatus} filterType={filterType} setFilterType={setFilterType} searchQuery={searchQuery} setSearchQuery={setSearchQuery} sortBy={sortBy} setSortBy={setSortBy} setSelectedList={setSelectedList} callListData={callListData} setCallListData={setCallListData} listFormOpen={listFormOpen} setListFormOpen={setListFormOpen} editingListId={editingListId} setEditingListId={setEditingListId} now={now} isAdmin={isAdmin} clientData={clientData} contactsByClient={contactsByClient} setCallFlowScreen={setCallFlowScreen} onOpenIndustryRules={() => setCurrentTab('rules')} />}
        ...
        {currentTab === "crm" && <CRMView isAdmin={isAdmin} clientData={clientData} setClientData={isAdmin ? setClientData : null} rewardMaster={rewardMaster} contactsByClient={contactsByClient} setContactsByClient={setContactsByClient} callListData={callListData} currentUser={currentUser} members={members} />}
        ...
        {currentTab === "overview" && isAdmin && <BusinessOverviewView appoData={appoData} callListData={callListData} clientData={clientData} setCurrentTab={setCurrentTab} setSelectedList={setSelectedList} setCallFlowScreen={setCallFlowScreen} />}
