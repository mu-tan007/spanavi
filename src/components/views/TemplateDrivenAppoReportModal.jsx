  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onSave?.({
        ...
        client:  list?.company || '',
        getter:  acquirerName,
        getDate: insResult?.appointment_date || form.getDate || '',
        meetDate: insResult?.meeting_date ? new Date(insResult.meeting_date).toLocaleDateString('en-CA', { timeZone: 'Asia/Tokyo' }) : form.appoDate || form.hearing_date || parsedMeet.date || '',
        meetTime: insResult?.meeting_time || form.appoTime || parsedMeet.time || '',
        meetLocation: insResult?.meeting_location || form.visitLocation || '',
        isOnline: insResult?.is_online ?? (form.meeting_format === 'オンライン' || form.meeting_format === 'Web'),
        client_id: insResult?.client_id || list?.client_id || null,
        list_id: insResult?.list_id || list?._supaId || null,
        item_id: insResult?.item_id || row?._supaId || row?.id || null,
        appoReport: reportNote,
        status:  initialAppoStatus(list),
        sales:   salesVal,
        reward:  rewardVal,
        month:   insResult?.appo_month || '',
        supaId: insResult?.id || null,
