...
export default function CareerDealsLostTab({ deals }) {
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {Object.entries(reasonCounts).map(([reason, count]) => (
            <div key={reason} style={{
              ...
              border: `1px solid ${C.border}`, borderRadius: 4, minWidth: 120,
            }}>
              <div style={{ color: C.textMid, fontSize: 10, marginBottom: 2 }}>{reason}</div>
              <div style={{ fontWeight: 700, color: C.navy, fontSize: 20, fontFamily: "'JetBrains Mono',monospace" }}>
                {count}<span style={{ fontSize: 11, marginLeft: 3, color: C.textMid }}>件</span>
              </div>
            </div>
          ))}
          ...
          <tbody>
            {lost.map(deal => (
              <tr key={deal.id} style={{ borderBottom: `1px solid ${C.borderLight}` }}>
                <td style={tdStyle}>{deal.prospect_name || '-'}</td>
