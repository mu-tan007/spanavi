...
export default function CareerDealsFunnelTab({ deals, stages }) {
  ...
  const visible = stageCounts.filter(s => !s.is_terminal || s.id === 'closed_won');
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {visible.map(stage => (
              <div key={stage.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontSize: 12, width: 180, color: C.textMid, flexShrink: 0 }}>{stage.label}</span>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, minWidth: 40, textAlign: 'right', fontFamily: "'JetBrains Mono',monospace", color: C.navy }}>{stage.count}</span>
              </div>
            ))}
          </div>
        </div>
        {lost > 0 && (
          <div style={{ marginTop: 10, fontSize: 11, color: C.textMid }}>※ 失注 {lost} 件は除外</div>
        )}
      </section>
    </div>
  );
