...
const sectionTitle = { fontSize: font.size.base, fontWeight: font.weight.semibold, color: color.navy, marginBottom: 4 }
const sectionHint = { fontSize: font.size.xs, color: color.textLight, marginBottom: 14, lineHeight: 1.6 }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <Card padding="lg">
        ...
        <div style={{ fontSize: font.size.xs, color: color.textLight, marginTop: 10 }}>※ 売上債権・買掛金・在庫データが別途必要 (試算表 or 外部DD取込)</div>
      </Card>
...
function Kpi({ label, value, sub }) {
  return (
    <Card padding="lg">
      <div style={{ fontSize: font.size.xs, color: color.textLight, marginBottom: 4, letterSpacing: 1 }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: font.weight.semibold, color: color.navy }}>{value}</div>
...
function MiniMetric({ label, value, unit }) {
  return (
    <div style={{ padding: 12, background: color.gray50, border: `0.5px solid ${color.border}`, borderRadius: radius.xl }}>
      ...
      <div style={{ fontSize: 16, fontWeight: font.weight.medium, color: color.navy }}>
        {value}{unit && value !== '—' ? <span style={{ fontSize: font.size.xs, color: color.textLight, marginLeft: 4 }}>{unit}</span> : null}
      </div>
    </div>
  )
}
...
function EmptyPlaceholder({ children }) {
  return (
    <div style={{ padding: '20px 16px', background: color.gray50, border: `0.5px dashed ${color.border}`, borderRadius: radius.lg, fontSize: font.size.sm, color: color.textLight, textAlign: 'center', lineHeight: 1.7 }}>
      {children}
    </div>
  )
}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: font.size.sm }}>
          <thead>
            <tr style={{ background: color.gray50 }}>
              {headers.map((h, i) => <th key={i} style={{ textAlign: i === 0 ? 'left' : 'right', padding: '8px 12px', fontSize: font.size.xs, color: color.textLight, fontWeight: font.weight.medium, borderBottom: `0.5px solid ${color.border}` }}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {rows.map((r, ri) => (
              <tr key={ri} style={{ background: ri % 2 === 0 ? color.white : color.gray50 }}>
                {r.map((c, ci) => <td key={ci} style={{ padding: '7px 12px', textAlign: ci === 0 ? 'left' : 'right', color: ci === 0 ? color.textLight : color.border, borderBottom: `0.5px solid ${color.borderLight}` }}>{c}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      {note && <div style={{ fontSize: font.size.xs, color: color.textLight, marginTop: 10, textAlign: 'center' }}>{note}</div>}
    </div>
  )
}
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: font.size.sm }}>
        <thead>
          <tr style={{ background: color.gray50 }}>
            <th style={{ textAlign: 'left', padding: '8px 12px', fontSize: font.size.xs, color: color.textLight, fontWeight: font.weight.medium, borderBottom: `0.5px solid ${color.border}` }}>項目</th>
            {financials.map(f => (
              <th key={f.id} style={{ textAlign: 'right', padding: '8px 12px', fontSize: font.size.xs, color: color.textLight, fontWeight: font.weight.medium, borderBottom: `0.5px solid ${color.border}` }}>
                {f.fiscal_year}年度
              </th>
            ))}
            <th style={{ textAlign: 'right', padding: '8px 12px', fontSize: font.size.xs, color: color.textLight, fontWeight: font.weight.medium, borderBottom: `0.5px solid ${color.border}` }}>直近YoY</th>
          </tr>
        </thead>
        <tbody>
          {rows.map(([label, key], ri) => {
            ...
            return (
              <tr key={key} style={{ background: ri % 2 === 0 ? color.white : color.gray50 }}>
                <td style={{ padding: '7px 12px', color: color.textLight, borderBottom: `0.5px solid ${color.borderLight}` }}>{label}</td>
                {financials.map(f => (
                  <td key={f.id} style={{ padding: '7px 12px', textAlign: 'right', color: color.navy, borderBottom: `0.5px solid ${color.borderLight}` }}>
                    {f[key] != null ? `¥${(f[key] / 100000000).toFixed(1)}億` : '—'}
                  </td>
                ))}
                <td style={{ padding: '7px 12px', textAlign: 'right', color: yoyVal == null ? color.border : yoyVal >= 0 ? color.success : color.danger, fontWeight: font.weight.medium, borderBottom: `0.5px solid ${color.borderLight}` }}>
