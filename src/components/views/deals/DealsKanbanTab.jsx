...
export default function DealsKanbanTab({ deals, stages, onCardClick, onStageChange }) {
  ...
  return (
    <div style={{ padding: 16, minHeight: '100%', overflowX: 'auto', background: C.offWhite }}>
      <div style={{
        display: 'grid',
        gridTemplateColumns: `repeat(${Math.max(stages.length, 1)}, 200px)`,
        gap: 10,
        minWidth: Math.max(stages.length, 1) * 210,
      }}>
        {stages.map(stage => {
          ...
          return (
            <div key={stage.id}>
              <div style={{
                ...
                <span style={{ color: C.navy, fontWeight: 600, fontSize: 11 }}>
                  {stage.label}
                </span>
                <span style={{ color: C.textMid, fontSize: 11, fontFamily: "'JetBrains Mono',monospace" }}>
                  {list.length}
                </span>
              </div>
